 ...
 *  - encrypt: Cypher algorithm to aply to the password (md5 or sha1) 
 *
...
class sspmod_sqlauth_Auth_Source_SQL extends sspmod_core_Auth_UserPassBase {
	...
	/**
	 * Cypher algorithm to aply to the password
	 *
	 */
	...
	private function connect() {
		...
		try {
			if($driver == 'pgsql') {
				$db = new PDO($this->dsn.' user='.$this->username.' password='.$this->password);
			}
			else {
