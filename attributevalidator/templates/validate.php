...
function present_attributes_table($t, $attributes, $nameParent, $color_missing_rows=False) {
    ...
    foreach ($attributes as $name => $data) {
        ...
        if (preg_match('/^child_/', $nameraw)) {
            ...
            if (sizeof($value) > 1) {
                $str .= '<tr class="' . $alternate[($i++ % 2)] . ' ' . $missingImg . ' ' . $color_row . '"><td class="attrmissing rightWhite"><span class="img'. $missingImg . '"/></td><td class="attrname">' . htmlspecialchars($name) . '</td><td class="attrvalue"><ul>';
